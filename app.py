from flask import Flask, render_template, request, send_from_directory, redirect, url_for, session, abort, current_app, flash, send_file
...
# ================== DOSYA TABANLI ZİYARETÇİ SAYACI ==================
COUNTER_PATH = os.path.join(DATA_DIR, "metrics.json")
_counter_lock = threading.Lock()

def _load_metrics():
    if not os.path.exists(COUNTER_PATH):
        return {"total": 0, "unique_ids": []}
    try:
...
def sayaci_arttir(path="/"):
    # Oturum kimliği yoksa ver
    sid = session.get("sid")
    if not sid:
        sid = session["sid"] = str(uuid.uuid4())

    # Gizlilik için hash’lenmiş, kırpılmış oturum izi
    sid_hash = hashlib.sha256(f"{sid}|{path}".encode("utf-8")).hexdigest()[:16]

    with _counter_lock:
        data = _load_metrics()
        # Toplam tıklama
        data["total"] = int(data.get("total", 0)) + 1
        # Unique (bu path için hash’i daha önce eklenmiş mi?)
        uniques = set(data.get("unique_ids", []))
        if sid_hash not in uniques:
            uniques.add(sid_hash)
            data["unique_ids"] = list(uniques)
        _save_metrics(data)

        total = data["total"]
        unique_visitors = len(data["unique_ids"])
    return total, unique_visitors
...
@app.route("/")
def giris():
    # Ana sayfa görüntülendi → sayaç kaydı
    total_hits, unique_visitors = sayaci_arttir(path="/")
...
def yapay_zeka_yorumla(yapan, test_edilen, alanlar):
    ...
    seed_src = (
        ...
        f"{session.get('cevap_id','')}|{int(time.time())//120}"
    )
...
def _yerel_serbest_yorum(ad, a, k, g, b, ton, profil=None, rng=None, vary=None, ipuclar=None):
    ...
    ikinci = max({k_:v for k_,v in vals.items() if k_ != birinci}, key=lambda z: vals[z])
    ...
    if rng.random() < 0.5:
        kapanis = (
            "Zaman içinde bu dört alanın dengesi değiştikçe iletişim sadeleşir ve karar süreçleri netleşir. "
            f"{ad} için öneri: düzenli görünürlük, merakla dinleme ve küçük pilotlar. "
            "Bu üçlü, doğal bir öğrenme hattı oluşturur."
        )
        return f"{giris}\n\n{gelisim}\n\n{kapanis}"
    else:
        return f"{giris}\n\n{gelisim}"
